...
function load_scripts()
{
    ...
    wp_enqueue_style('theme-style-base', mySrc() . '/css/base.css', '20180518');
    wp_enqueue_style('theme-style', mySrc() . '/style.css', '20180518');
    wp_enqueue_style('index', mySrc() . '/css/index.css', '20180518');
    wp_enqueue_style('swiper', mySrc() . '/swiper/swiper.min.css', '20180518');
    
    wp_enqueue_script('site_swiper', mySrc() . '/swiper/swiper.min.js', '', '', true);
...
function footer_nav_menu(){
    $defaults = array(
        'theme_location' => 'footer_menu',//用于在调用导航菜单时指定注册过的某一个导航菜单名，如果没有指定，则显示第一个
        'container' => 'div',//最外层容器标签名
        'container_class' => 'Relevant-link-list',//最外层容器class名
        'container_id' => '',//最外层容器id值
        'menu_class' => 'footer-menu',//ul 节点的 class 属性值。
        'menu_id' => 'footer-menu',//ul 节点的 id 属性值。
